...
mlevan-oss is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i mlevan-oss:latest
   ```
...
kubectl get mcpserver mlevan-oss
```